...
if(isset($_FILES["FileInput"]) && $_FILES["FileInput"]["error"]== UPLOAD_ERR_OK)
{
 ...
 {  
      ...
      {  
           $html.="<tr>";  
           $item = $worksheet->getCellByColumnAndRow(0, $row)->getValue();  
           ...
           $popup =  $worksheet->getCellByColumnAndRow(5, $row)->getValue();
           $price =  $worksheet->getCellByColumnAndRow(6, $row)->getValue();
           $result=$mysqli_object->query("SELECT * FROM `rstock` where shopemail='$email' and item='$item'");
           
           if($result->num_rows===1)
           {
               if($mysqli_object->query("UPDATE `rstock` SET qty=qty+'$qty' , popup='$popup' ,price='$price' where shopemail='$email' and item='$item' ")==true);

           }
        else {
            if($mysqli_object->query("INSERT INTO `rstock`(`shopemail`, `item`, `brand`, `size`, `unit`, `qty`, `popup`,`price`) VALUES('" .$email . "','" . $item . "','" . $brand . "','" . $size . "','" . $unit . "','" . $qty . "','" . $popup . "','" . $price . "')")==true);
