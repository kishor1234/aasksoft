...
if(isset($_POST['installer_submitted'])) {
	...
	if(!$error) {
		if(!@mysql_connect($_POST['db_hostname'], $_POST['db_username'], $_POST['db_password']))
                {
                    $error .= 'Your database host, username, or password information were not correct.<br />';
		
                }
                else
                {
                    $conn = new mysqli($_POST['db_hostname'], $_POST['db_username'], $_POST['db_password']);
                    // Check connection
	...
	<body>
		<div id="content">
				...
				<?php }else { ?>
					...
					<?php (isset($error) && $error?'<div class="alert alert-danger">'.$error.'</div>':''); ?>

					<hr />

					<?php $dirperms = substr(sprintf('%o', fileperms('include')), -4); ?>
					<?php if($dirperms != '0777' && $dirperms != '0755' && !chmod("include", 0755)) { ?>
